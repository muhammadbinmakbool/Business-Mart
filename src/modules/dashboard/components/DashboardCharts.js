...
export function DailyActivityChart({ data = [] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="h-[160px] bg-slate-950/40 rounded-xl p-3 border border-slate-900 flex items-end justify-between gap-2">
        {data.length > 0 ? (
          data.map((day, idx) => {
            ...
            return (
              <div key={idx} className="flex-1 flex flex-col items-center h-full group relative">
                {/* Tooltip */}
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 bg-slate-900 border border-slate-800 rounded px-2 py-1 text-[9px] font-mono text-slate-200 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 whitespace-nowrap shadow-lg">
                  <div className="font-bold border-b border-slate-800 pb-0.5 mb-0.5">{day.date}</div>
                  <div className="text-sky-400">Intakes: {day.intakeCount}</div>
                  <div className="text-violet-400">Sales: {day.saleCount}</div>
                </div>
            ...
            return (
              <div key={idx} className="space-y-1">
                <div className="flex items-center justify-between text-xs font-semibold">
                  ...
                  <span className="text-slate-400 font-mono">{item.weight.toLocaleString()} KG</span>
                </div>
                <div className="h-2 w-full bg-slate-900 border border-slate-800/40 rounded-full overflow-hidden">
                  <div 
                    style={{ width: `${pct}%` }}
          ...
          data.map((day, idx) => {
            ...
            return (
              <div key={idx} className="flex-1 flex flex-col items-center h-full group relative">
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 bg-slate-900 border border-slate-800 rounded px-2 py-1 text-[9px] font-mono text-slate-200 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 whitespace-nowrap shadow-lg">
                  <div className="font-bold border-b border-slate-800 pb-0.5 mb-0.5">{day.date}</div>
                  <div className="text-amber-400">Settlement: Rs. {day.settlementValue.toLocaleString()}</div>
                  <div className="text-teal-400">Sales: Rs. {day.saleValue.toLocaleString()}</div>
                </div>
                ...
                <div className="flex-1 w-full flex items-end justify-center gap-0.5">
                  <div 
                    ...
                    className="w-1.5 bg-amber-500/80 hover:bg-amber-400 rounded-t transition-all"
                  />
                  <div 
                    style={{ height: `${saleHeight}%` }} 
                    className="w-1.5 bg-teal-500/80 hover:bg-teal-400 rounded-t transition-all"
                  />
                </div>
                <span className="text-[9px] font-mono text-slate-500 mt-1 select-none">
                  {day.label}
          ...
          data.map((s, idx) => (
            <div 
              ...
              className={`p-2.5 rounded-lg border flex flex-col justify-between items-center text-center space-y-1.5 transition-all shadow-sm ${
                s.matched 
                  ? "bg-emerald-950/20 border-emerald-950/60 hover:bg-emerald-950/30" 
                  : "bg-rose-950/20 border-rose-950/60 hover:bg-rose-950/30"
              }`}
              title={`${s.title} (Diff: Rs. ${s.difference})`}
            >
              <span className="text-[9px] font-bold text-slate-400 truncate max-w-[60px] block font-mono">
              ...
              <span className={`text-[8.5px] font-bold font-mono px-1 py-0.5 rounded ${
                s.matched ? "text-emerald-400 bg-emerald-950/60" : "text-rose-400 bg-rose-950/60"
              }`}>
